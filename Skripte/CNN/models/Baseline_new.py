...
def baseline_new(x, keep_prob, is_train, conf):
    ...
    #             if i == 0:  # very first layer
    #                 net = new_conv1d_layer(x_multichannel, kernels*(i+1), firstlayer_extent, 'conv_'+str(i))
    #             else:
    #                 net = new_conv1d_layer(net, kernels*2**i, layer_extent, 'conv_'+str(i))
    #
    #             for j in range(add_conv):
    #                 net = new_conv1d_layer(net, kernels*2**i, layer_extent, 'conv_' +str(i) + '_' + str(j+1))
    #             net = tf.layers.max_pooling1d(net, 2, 2, name='maxpool'+str(i))
    ...
    #         with tf.variable_scope('Stage_' + str(i)):
    #             if i == 0:  # very first layer
    #                 net = new_conv2d_layer(x_multichannel, kernels*(i+1), firstlayer_extent, 'conv_'+str(i))
    #             else:
    #                 net = new_conv2d_layer(net, kernels*2**i, layer_extent, 'conv_'+str(i))
    #
    #             for j in range(add_conv):
    #                 net = new_conv2d_layer(net, kernels*2**i, layer_extent, 'conv_' +str(i) + '_' + str(j+1))
    #             net = tf.layers.max_pooling2d(net, 2, 2, name='maxpool'+str(i))
    #     net = tf.layers.flatten(net, 'flat_layer')
    #     net = new_fc_layer(net, 128, "fc1_relu_bn")
    #     # apply dropout to first FC layer
    #     net = tf.nn.dropout(net, keep_prob, name='Dropout')
