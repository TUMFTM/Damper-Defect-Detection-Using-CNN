...
prep1d = ['None', 'detrend', 'standard', 'zero2one', 'neg_one2one', 'fourier_channels', 'gaussian']
...
# config.input = '2D'
# config.preprocessing = 'gaf'
config.model = 'paper_prep'
# config.preprocessing = 'fourier_channels' if config.input == '1D' else 'stft'
...
def main(argv=None):  # pylint: disable=unused-argument
    if config.evaluate:
        ...
        # prep2d = ['greyscale', 'stft', 'gaf', 'recurrence_plot', 'wpi']
        prep2d = ['gaf', 'recurrence_plot']
        # prep1d = ['None', 'detrend', 'gaussian', 'standard', 'neg_one2one', 'fourier_channels', 'fourier_samples']
        prep1d = ['fourier_samples']
        if config.input == '1D':
            prep = prep1d
        else:
            prep = prep2d
        ...
        for config.preprocessing in prep:
            ...
            alltrain, test_set, labels, config.num_ch, config.seq_lngth, config.sensors, config.img_dim, scaler = \
                datahandler.handle_data(config.datapath, config.sel_sensors,
                                        file_format='csv', preprocessing_type=config.preprocessing,
                                        split_ratio=config.test_split)
    ...
    # print("Final test accuracy: %s" % result) # wieder einkommentieren
