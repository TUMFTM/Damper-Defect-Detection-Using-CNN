...
# Buduma.2017:  https://github.com/darksigma/Fundamentals-of-Deep-Learning-Book/blob/master/archive/convnet_mnist.py
...
def run(conf, dataset, classlabels):
    ...
    with tf.variable_scope("CNN_Model"):
        ...
        # need to handle different from train set (no shuffling, in last mini batch only use the remaining few samples)
        testacc = 0.
        test_predictions = []
        ...
        confmat = evaluate_metrics(dataset, test_predictions, classlabels, conf.metrics_avg)
...
def evaluate_metrics(dataset, prediction, labels, average, print_it=True):
